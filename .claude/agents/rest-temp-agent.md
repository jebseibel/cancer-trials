...
13. `DomainBuilderSystemDatabase.java` - `test/java/com/quokka/testutils` (append builder methods)
14. Liquibase changeset YAML - `database/src/main/resources/db/changelog/changes/` (create table + add to master changelog)
...
- `{Entity}Mapper.java` - Object conversions
    - Uses `@Component`, `@NoArgsConstructor`
        ...
        - No dependency injection needed (ModelMapper is stateless)
...
### 14. **DomainBuilderSystemDatabase** (`test/.../testutils`)
...
### 15. **Liquibase Changeset** (`database/src/main/resources/db/changelog/changes/`)
- YAML changeset file for creating the database table
- Follow existing numbering convention (e.g., `029-customer-table.yaml`)
- Must be added to `db.changelog-master.yaml`

**Changeset pattern (using Customer as example):**
      ...
      changes:
        ...
        - createTable:
            ...
            columns:
              ...
              - column: { name: code, type: varchar(8), constraints: { nullable: false, unique: true } }
...
- `extid` - varchar(36), UUID default, unique, not null
- Business fields - from Domain class with snake_case naming
...
**Column type mapping:**
- `String` → `varchar(length)` with length from field constraint
- `Integer`/`int` → `int`
- `Long`/`long` → `bigint`
...
9. Controller class
10. Converter class (separate file)
11. Mapper Test class
12. Repository Test class
13. Database Service Test class
14. **DomainBuilderSystemDatabase methods** (append to existing shared class)
...
Entity-specific overrides to the standard generation pattern:

| Entity | Field | Note |
|--------|-------|------|
| CheckinMoodTypeDb | code | Display only — included in Response DTO but excluded from RequestCreate and RequestUpdate |
| ProfileStageTypeDb | code | Display only — included in Response DTO but excluded from RequestCreate and RequestUpdate |
