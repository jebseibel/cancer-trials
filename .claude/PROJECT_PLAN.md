...
[`TABLES.md`](./TABLES.md)). This means:
...
[`TABLES.md`](./TABLES.md), tracked and iterated on separately from this plan.
...
the source registry (see `TABLES.md`). The normalization job doesn't care where a
...
- They connect directly to the same local MySQL database and insert rows into the
  staging table (schema in `TABLES.md`) with a JSON payload shaped however is natural
...
2. Core schema as Liquibase changesets (see `TABLES.md` for the table design)
