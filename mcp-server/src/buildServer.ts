/**
 * Builds one McpServer instance with all tools registered. Factored out of index.ts so both
 * the stdio entry point (local, single-user) and the HTTP entry point (remote, per-session)
 * can share the exact same tool definitions — the transport is the only thing that differs.
 */
