...
export const inputClass =
    'w-full px-3 py-2 border border-stone-300 rounded-md shadow-sm text-sm focus:outline-none focus:ring-brand-green focus:border-brand-green';
...
export function Field({
    ...
    return (
        <div className={className}>
            ...
            {children}
            {/* Hints are sentences a reader reads carefully while filling in her own record,
                not a metadata tag - text-base per UI_DESIGN.md §5's 16px floor, not text-xs. */}
            {hint && <p className="mt-1 text-base text-stone-500 leading-normal">{hint}</p>}
        </div>
    );
}
