...
import twoLadiesImage from '../assets/images/two-ladies-with-ribbons-long.png';
...
function TrialSites({ assessment }: { assessment: TrialAssessment }) {
    ...
    if (!siteCities || siteCities.length === 0) {
        return (
            <div className="mt-1.5 flex items-start gap-1.5 text-sm text-stone-500">
    ...
    return (
        <div
            className={`mt-1.5 flex items-start gap-1.5 text-sm ${
                hasUnitedStatesSite ? 'text-stone-700' : 'text-amber-800'
            }`}
        >
            <MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            <div>
...
function AssessmentCard({ assessment }: { assessment: TrialAssessment }) {
    ...
    return (
        <div className="rounded-lg border border-stone-200 bg-brand-beige-card p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                    ...
                    >
                        {displayTitle(assessment, mode) || assessment.nctId}
                    </Link>
                ...
                <div className="flex w-full shrink-0 flex-wrap gap-x-3 gap-y-1 text-xs text-stone-600 sm:w-auto">
                    ...
                    {passed.length > 0 && (
                        <span className="text-stone-500">{passed.length} matched</span>
                    )}
                </div>
            </div>
            ...
            {notable.length === 0 && (
                <p className="mt-3 text-base text-stone-600 leading-normal">
            ...
            {passed.length > 0 && (
                <div className="mt-3">
                    <button
                        ...
                        className="inline-flex min-h-8 items-center gap-1 py-1 text-xs text-stone-500 hover:text-stone-700"
                    >
                    ...
                    {showAll && (
                        <ul className="mt-2 space-y-1">
                            {passed.map((s) => (
                                <li key={s.name} className="flex items-start gap-2 text-sm text-stone-600">
    ...
    return (
        <div>
            <div className="mx-auto max-w-4xl">
                <div className="mb-6">
                    <h1 className="font-heading flex items-center gap-2 text-2xl font-semibold text-stone-900">
                        <ListChecks className="h-6 w-6 text-brand-green" />
                        Trials for You
                    </h1>
                    <p className="mt-1 text-base text-stone-600 leading-normal">
            ...
            {/* A companion image for "this is a starting point" below - free stock photography
                with both women's faces out of frame, so nothing here is identifiable as any
                real person. See UI_DESIGN.md §2 for the test this was checked against.
                max-w-4xl to match the text column above it exactly, rather than the page's
                wider max-w-7xl - it previously ran wider than the title/intro text it sits
                under. Placed below the title/intro text, above the no-verdicts callout. */}
            <img
                src={twoLadiesImage}
                alt=""
                className="mb-6 w-full max-w-4xl mx-auto h-40 sm:h-56 rounded-lg object-cover shadow-md"
            />

            <div className="mx-auto max-w-4xl">
                {/* Said plainly and up front, because it is the one thing a reader could most
                    easily get wrong about what this page is. */}
                <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-base text-amber-900 leading-normal">
                ...
                <div className="mb-6 flex flex-wrap items-center gap-3">
                    <button
                        ...
                        className="rounded-md bg-brand-green px-4 py-2 text-white hover:bg-brand-green-hover disabled:opacity-50"
                    >
                ...
                {rank.isPending && (
                    <div className="rounded-lg border border-stone-200 bg-brand-beige-card p-6 text-center text-base text-stone-600 leading-normal">
                        Checking each trial against your record. This can take up to a minute.
                    </div>
                )}

                {rank.isError && (
                    <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-base text-red-800 leading-normal">
                        Something went wrong while looking through the trials. Please try again.
                    </div>
                )}

                {rank.isSuccess && results.length === 0 && (
                    <div className="rounded-lg border border-stone-200 bg-brand-beige-card p-6 text-base text-stone-600 leading-normal">
